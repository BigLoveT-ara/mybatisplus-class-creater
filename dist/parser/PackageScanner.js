...
export class PackageScanner {
     ...
     * @returns 包名信息
     */
    inferPackageInfo(scanDir, classes) {
        // 从第一个类获取包名
        if (classes.length === 0) {
            return {
                entityPackage: '',
     ...
     * @returns 基础包名
     */
    inferBasePackage(entityPackage) {
        if (!entityPackage) {
            return '';
        }
     ...
     * @returns 扫描结果
     */
    scanFromProjectRoot(projectRoot) {
        ...
        if (!entityDir) {
            throw new Error(`未找到 Entity 目录，请确保存在 src/main/java/{package}/entity 或类似结构的目录`);
        }
 ...
 * 创建包扫描器
 */
export function createPackageScanner() {

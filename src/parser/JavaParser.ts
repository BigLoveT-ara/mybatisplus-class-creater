...
import { JavaClass, JavaField, JavaMethod, Annotation, ImportStatement, MethodParameter } from '../types.js';
...
export class JavaParser {
   ...
   * 提取导入语句
   */
  private extractImports(): ImportStatement[] {
   ...
   * 提取类注释
   */
  private extractClassComment(): string | undefined {
    // 先找到类定义的位置
   ...
   * 提取类注解
   */
  private extractClassAnnotations(): Annotation[] {
   ...
   * 提取父类
   */
  private extractSuperClass(): string | undefined {
   ...
   * 提取字段
   */
  private extractFields(): JavaField[] {
   ...
   * 提取方法
   */
  private extractMethods(): JavaMethod[] {
   ...
   * 解析方法参数
   */
  private parseParameters(paramStr: string): MethodParameter[] {
   ...
   * 解析注解
   */
  private parseAnnotations(annotationStr: string): Annotation[] {
   ...
   * 解析注解属性
   */
  private parseAnnotationAttributes(attributesStr: string): Record<string, string> {
   ...
   * 清理注释文本
   */
  private cleanComment(commentStr: string): string {
    return commentStr
      .replace(/\/\*\*|\*\//g, '')
      .replace(/^\s*\*\s*/gm, '')
      .split('\n')
   ...
   * 移除方法体（避免在方法内部匹配字段）
   */
  private removeMethodBodies(): string {
    let result = '';
    let braceCount = 0;
    let inMethod = false;
    let i = 0;
 ...
 * 解析 Java 文件
 */
export function parseJavaFile(filePath: string): JavaClass {
 ...
 * 解析目录下的所有 Java 文件
 */
export function parseJavaDirectory(dirPath: string): JavaClass[] {
